...
import libtcodpy
import tcod.event
from tcod.event import KeyDown, MouseMotion
...
class HistoryViewer(EventHandler):
    """Print the history on a larger window to be navigated"""
    def __init__(self, engine: Engine):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> None:
        ...
        if event.sym in CURSOR_Y_KEYS:
            ...
            if adjust < 0 and self.cursor == 0:
                ...
                #Otherwise, move while staying clamped to the bounds of history log.
                self.cursor = max(0, min(self.cursor + adjust, self.log_length -1))
        elif event.sym == tcod.event.KeySym.HOME:
            self.cursor = 0 #Move directly to the top message.
        elif event.sym == tcod.event.KeySym.END:
            self.cursor = self.log_length -1 #Move directly to the lat message.
        else:
            self.engine.event_handler = MainGameEventHandler(self.engine) #Move directly to the top message.    
