...
class RectangularRoom:
    ...
    def intersects(self, other: RectangularRoom) -> bool:
        """Return true if this room overlap with another room"""
        return(
...
def generate_dungeon(max_rooms: int, room_min_size: int, room_max_size: int, map_width: int, map_height: int, max_monster_per_rooms: int, engine: Engine) -> GameMap:
    ...
    for r in range(max_rooms):
        ...
        #RectangularRoom class makes rectangle room easier to work with
        new_room = RectangularRoom(x, y, room_width, room_height)

        #Run through other room to see if they intersect
        if any(new_room.intersects(other_room) for other_room in rooms):
            continue #This room intersect, so go to the next attempt
        #If there are no intersection then the room is valid

        #Dig out the room floor
        dungeon.tiles[new_room.inner] = tile_types.floor

        if len(rooms) == 0: #The first room is the one where the player starts
