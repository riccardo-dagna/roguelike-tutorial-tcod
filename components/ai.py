...
from actions.actions import Action, BumpAction, MeleeAction, MovementAction, WaitAction
...
class StunnedEnemy(BaseAI):
    """
    A stunned enemy will wait for x turns and then it will revert to the original AI.
    """
    ...
    def perform(self) -> None:
        # Revert the AI back to the original state if the effect has run its course
        if self.turns_remaining <= 0:
            self.engine.message_log.add_message(f"The {self.entity.name} is no longer stunned.")
            self.entity.ai = self.previous_ai
        else:
            self.turns_remaining -= 1
            return WaitAction(self.entity).perform()
