...
class MessageLog:
    ...
    def add_message(self, text: str, fg: Tuple[int, int, int] = color.white, *, stack: bool = True) -> None:
        """Add a message to the log. If 'stack' is true, the message can stack with a previous message of the same text"""
    ...
    @classmethod
    def render_messages(cls, console: tcod.console.Console, x: int, y: int, width: int, height: int, messages: Reversible[Message]) -> None:
        """Render the messages provided. It starts at the last and works backwards."""
        y_offset = height - 1
